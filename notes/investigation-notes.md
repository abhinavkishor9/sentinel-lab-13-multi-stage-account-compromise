# Investigation Notes — Multi-Stage Account Compromise

## Investigation Summary

The investigation examined simulated identity activity associated with `user1@contoso.com`.

Three failed authentication attempts were recorded from `10.10.10.25` in Hyderabad. Four minutes after the final failure, the account successfully authenticated from `203.0.113.20`, represented in the dataset as New York, US.

Three minutes later, an authentication method was added to the account. The account then accessed the Azure Portal and viewed resources from the same New York source IP.

The individual events do not independently prove account compromise. Their sequence and timing create a suspicious multi-stage pattern that requires further validation.
...
The simulated evidence forms a coherent suspicious identity-event sequence involving `user1@contoso.com`.

The main investigative value comes from the relationship between the events:
...
The sequence warrants further investigation, but the available telemetry alone is insufficient to confirm that the account was compromised.
